...
- **动态帧率自适应**：8 种无特权硬件遥测源（CPU、内存+Swap、GPU、网络、磁盘、风扇、电池放电电流、芯片结温）驱动状态栏 GIF 变速播放。
...
| 文种 | 命名规范 | 职责与生命周期 |
|---|---|---|
| **路线图** | `docs/ROADMAP.md` | 悬着的议题总表（记录在案的 NO、候选议题 `R<n>`、排期、验证债务）；只记决定与条件，不记流水账；议题落成即移出 |
...
- **8 种无特权硬件遥测源** (`docs/ARCHITECTURE.md` §4)：CPU (Mach)、Memory + Swap (Mach `vm_statistics64`)、GPU (IOAccelerator)、Network/Disk (IOKit 计数器增量)、Fan RPM (SMC)、Battery mA (IOKit PS)、Max Die Temp (SMC 二分查找 `Tp**`/`Tpx*` 传感器集群最大值)。每种源均具备 `isAvailable` 探测，不可用时平滑降级。
...
- **未覆盖功能必须记录在 `docs/ROADMAP.md` 验证债务表**，严禁谎称覆盖。
