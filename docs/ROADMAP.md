...
| R10 | **GPU power / ANE / package power readers.** The one tier needing a private, unheadered API (`IOReport`), which every current reader avoids. Also the first needing a long-lived subscription rather than a point read — its own design pass, not an add-a-reader task. Assessed 2026-08-02, refreshed 2026-09-11: `mac-vitals` demonstrates unprivileged Apple Silicon Energy Model rail reads (CPU/GPU/ANE in Watts via `IOReport`) without root; `featherbar` and `light-stats` affirm developer interest in live wattage. However, ROI remains constrained — only ANE is a genuinely new signal (GPU power tracks the utilization reader; package power correlates with CPU/temperature, and battery mA already shows whole-system draw) against a permanent private-API maintenance surface and per-chip channel discovery debt. Full feasibility, channel subscription architecture, and gating criteria: [`PLAN-power-readers.md`](PLAN-power-readers.md). | P4 | — |
...
| Item | Why not |
|---|---|
| Periodic update polling (R18) | **Decided 2026-09-10: launch-time discovery plus “Check for Updates…” on demand is sufficient.** Releases are not time-critical, and login-item users are checked at every login or restart rather than being unreachable. Recurring background network checks would add interval policy and lifecycle complexity for little benefit. Reconsider only with a concrete report of materially stale long-running installations or if updates become security-sensitive. |
...
| Embedded Model Context Protocol (MCP) server or background daemon (like mac-vitals) | **Violates the unbundled, single-process, CLI-first architecture.** An embedded JSON-RPC / stdio MCP server introduces socket lifecycles, protocol parsers, and background thread concurrency into a zero-daemon status-bar visualizer. Coding agents already integrate with MenuBar Load Runner through standard Unix interfaces: checking status via CLI/argv, inspecting `state.json`, and keeping the Mac awake during agent runs via `--keep-awake-pid $PPID`. |
