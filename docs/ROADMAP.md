...
| Pre-flight admission gate — `--check` (proposed with R27) | **The threshold is the caller's, and the verdict would corrupt a test hook.** What counts as "too hot to start" is a property of the job about to run, not of the observed machine; the gate would also have reused the keep-awake battery threshold — a sleep-prevention intent — as an admission threshold, conflating two numbers. Worse, its verdict reads `KernelThermalPressure`, which `docs/ARCHITECTURE.md` §10 holds display-only precisely so `CO_AWARENESS_FORCE_THERMAL` stays an input simulator rather than a hook that moves a business decision. `--once` already publishes `thermal` and `battery_pct`. Re-propose as the caller's own threshold applied by the caller. |
...
| First-class local custom-GIF presets / runtime import (R9) | Over-design: custom GIF import is not a core feature or user requirement. The system only requires a reproducible SOP to add whatever art is needed (add optimized GIF to `gifs/` and register in `gifs/presets.json`, per `AGENTS.md`); a raw positional path (`./co-awareness /path/to.gif`) remains the unbundled escape hatch. |
